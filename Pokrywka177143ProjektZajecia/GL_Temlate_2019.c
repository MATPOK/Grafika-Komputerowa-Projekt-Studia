...
float angleX = 0.0f;  // Kąt obrotu w osi X
float angleY = 0.0f;  // Kąt obrotu w osi Y
...
//DOPISANE NA ZAJ TWORZENIE 2 ŚCIAN SZESCIAKATA
void szescian(void)
{
	glBegin(GL_QUADS);
	glColor3d(1, 0.5, 0);
...
void RenderScene(void)
{
	...
	glFogf(GL_FOG_START, 5.0f); // Start mgły (dla GL_LINEAR)
	glFogf(GL_FOG_END, 20.0f);  // Koniec mgły (dla GL_LINEAR)
	...
	LPARAM  lParam)
{
	...
	case WM_CREATE:
		...
		// ładuje trzeci (puma) obraz tekstury:
		...
		// ładuje czwarty (trawa) obraz tekstury:
	...
	case WM_TIMER:
		...
		case WM_MOUSEMOVE:
		{
			// Odczytanie współrzędnych myszy
			int x = LOWORD(lParam);  // Współrzędna X
			int y = HIWORD(lParam);  // Współrzędna Y

			// Oblicz różnicę w pozycjach myszy
			int dx = x - lastX;
			int dy = y - lastY;

			// Zaktualizuj kąty obrotu na podstawie różnicy pozycji
			angleX += dy * 0.1f;  // Obrót w osi X
			angleY += dx * 0.1f;  // Obrót w osi Y

			// Zapisz ostatnią pozycję myszy
			lastX = x;
			lastY = y;
